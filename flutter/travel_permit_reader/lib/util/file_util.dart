...
class FileUtil {
  static Future<File> createFromResponse(Response download, String defaultName, bool isTemp) async {
    final name = getFilenameFromHeader(download.headers['content-disposition']) ?? defaultName;
  ...
  static Future<String> getFileUniquePath(String path) async {
    final dirPlusName = p.join(p.dirname(path), p.basenameWithoutExtension(path));
